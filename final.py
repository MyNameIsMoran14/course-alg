...
class AVLNode:
    def __init__(self, key, r_type, seats, rooms, toilet, equipment):
        ...
        self.seats, self.rooms, self.has_toilet = seats, rooms, toilet # ТЗ поля
...
class HashNode:
    def __init__(self, passport, fio, birth, address, goal, checkout_date="Проживает"):
        ...
        self.birth, self.address, self.goal = birth, address, goal # ТЗ поля
        ...
        if not self.table[idx]: self.table[idx] = HashNode(passport, fio, birth, address, goal, date)
            ...
            while curr:
                if curr.passport == passport:
                    curr.fio, curr.checkout_date = fio, date
                    return
...
def main(page: ft.Page):
    ...
    cur = conn.cursor()
    # Обновляем структуру таблиц под требования ТЗ
    ...
    def reload_all_data():
        ...
        for r in cur.fetchall(): 
            tree_root[0] = tree_engine.insert(tree_root[0], r[0], r[1], r[2], r[3], r[4], r[5])
        cur.execute("SELECT * FROM guests")
        for g in cur.fetchall(): 
            hash_engine.insert(g[0], g[1], g[2], g[3], g[4], g[5])
    ...
    def show_rooms(e=None):
        ...
        rid = ft.TextField(label="№ (ANNN)", width=110) # ТЗ формат
        rtype = ft.TextField(label="Тип (Л,П,О,М)", width=110)
            ...
            for n in nodes:
                if not filter_str or boyer_moore_search(n.equipment.lower(), filter_str.lower()):
                    res_list.controls.append(
                        ft.Row([
                            ft.Text(f"[{n.key}] {n.room_type} | Мест: {n.seats} | Комн: {n.rooms} | С/У: {'Да' if n.has_toilet else 'Нет'}\nОборудование: {n.equipment}", expand=True),
                            ft.IconButton(ft.Icons.DELETE, tooltip="Удалить номер", 
                                          on_click=lambda _, r=n.key: delete_room_full(r))
                        ])
                    )
            page.update()

        def add_ev(e):
            # Проверка формата ANNN по ТЗ
            if not re.match(r"^[ЛПОМ]\d{3}$", rid.value):
                page.snack_bar = ft.SnackBar(ft.Text("Формат номера: Буква(Л,П,О,М) и 3 цифры!")); page.snack_bar.open = True
                page.update(); return
            
            tree_root[0] = tree_engine.insert(tree_root[0], rid.value, rtype.value, int(rseats.value or 1), int(rcnt.value or 1), int(rtoilet.value), requip.value)
            cur.execute("INSERT OR REPLACE INTO rooms VALUES (?, ?, ?, ?, ?, ?)", 
                        (rid.value, rtype.value, int(rseats.value or 1), int(rcnt.value or 1), int(rtoilet.value), requip.value))
    ...
    def show_guests(e=None):
        content_area.controls.clear()
        search_g = ft.TextField(label="Поиск по ФИО или паспорту (БМ)", width=400)
                        ...
                        res_list.controls.append(
                            ft.Container(
                                content=ft.Row([
                                    ...
                                    ft.Text(f"{curr.fio} ({curr.passport}) | {curr.birth}г. | {curr.address}\nЦель: {curr.goal}", expand=True), 
        ...
        content_area.controls.extend([
            ft.Row([search_g, ft.FilledButton("Найти", on_click=lambda _: update_ui(search_g.value))]),
            ft.Divider(), res_list
        ])
        update_ui()
    ...
    def show_journal(e=None):
        ...
        jpass = ft.TextField(label="Паспорт (NNNN-NNNNNN)", width=180)
        jfio = ft.TextField(label="ФИО", width=150)
        jbirth = ft.TextField(label="Год рождения", width=100)
        jaddr = ft.TextField(label="Адрес", width=150)
        jgoal = ft.TextField(label="Цель", expand=True)
        ...
        def add_ev(e):
            ...
            # Валидация паспорта по ТЗ
            if not re.match(r"^\d{4}-\d{6}$", jpass.value):
                page.snack_bar = ft.SnackBar(ft.Text("Формат паспорта: 1234-123456")); page.snack_bar.open = True
                page.update(); return
